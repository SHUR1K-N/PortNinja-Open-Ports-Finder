...
def createThreads(checkedURL):
    ...
    if (method in ["2", "4"]):
        if (method == "2"):
            print(f"\nScanning \"{URL}\" for top 1,000 common ports...\n")
        if (method == "4"):
...
def processURL(URL):
    print("\n\nChecking to see if URL exists...", end="")
    verification = requests.get(URL)
    if verification.status_code == 200:
        print(colored(" Success!", "green")), time.sleep(0.7)
        time.sleep(0.5)
        clrscr()
    else:
        print(colored(" Failed!", "red")), time.sleep(0.7)
        raise Exception
    ...
    while (True):
        ...
        if (method == "1"):
            ...
            while (True):
                ...
                portMax = int(input("Enter ending port here: "))
                scanSpeed = speedPrompt()
